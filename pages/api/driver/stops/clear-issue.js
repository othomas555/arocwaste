// pages/api/driver/stops/clear-issue.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { error: eUpdate } = await supabase
      ...
      .update({ resolved_at: new Date().toISOString(), resolved_by_staff_id: staffRow.id })
      .eq("run_id", run_id)
      .eq("stop_type", stop_type)
      .eq("stop_id", stop_id)
      .is("resolved_at", null);

    if (eUpdate) return res.status(500).json({ error: eUpdate.message });

    return res.status(200).json({ ok: true });
  } catch (e) {
    return res.status(500).json({ error: e?.message || "Failed to clear issue" });
  }
}
